<template>
    <div class="w-full bg-purple-900 rounded">
        ...
        <div class="bg-gray-900 p-4 rounded m-2 w-1/6 text-center" v-for="stream in data">
            <img :src="`https://static-cdn.jtvnw.net/previews-ttv/live_user_${stream.user_name}-272x160.jpg`"
                 :alt="`${stream.user_name}s Stream Thumbnail`" style="width: 270px" class="mx-1 rounded text-center">
            ...
            &middot;
            <a :href="`https://twitch.tv/${stream.user_name}`" target="_blank" class="no-underline text-gray-200 hover:text-gray-300 hover:underline">View</a>
        </div>
    </div>
</template>
